...
if ($force_logout && isset($_SESSION['user_id'])) {
    ...
    header('Location: login_superadmin.php' . $tab_param);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $db_configured) {
    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            // Find user with superadmin role - case sensitive username match
            $sql = "SELECT id, username, password, full_name, email, role, status, profile_picture 
                    FROM users 
                    WHERE BINARY username = ? AND role = 'superadmin' AND status = 'active'";
            ...
            if ($user) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    // Log activity dengan deteksi keamanan
                    log_activity_with_detection($user['id'], 'LOGIN_SUPERADMIN', 'Superadmin berhasil login');
                ...
                if ($check_user) {
                    if ($check_user['role'] !== 'superadmin') {
                        $error_message = 'Username ini bukan superadmin. Silakan login sebagai admin atau staff.';
...
<head>
    ...
    <title>Login Superadmin - Sistem Arsip Dokumen</title>
...
<body class="login-body">
    <div class="container-fluid">
        <div class="row min-vh-100">
            <!-- Side Image -->
            <div class="col-lg-6 d-none d-lg-flex align-items-center justify-content-center" style="background: linear-gradient(135deg, #DC2626 0%, #EF4444 100%);">
                <div class="text-center text-white">
                    <i class="fas fa-user-shield fa-5x mb-4"></i>
                    <h2 class="mb-3">Superadmin Dashboard</h2>
                    <h4 class="mb-4">Sistem Arsip Dokumen</h4>
                    <p class="lead">Akses penuh ke semua fitur sistem</p>
                </div>
            </div>
            
            <!-- Login Form -->
            <div class="col-lg-6 d-flex align-items-center justify-content-center" style="background: linear-gradient(135deg, #DC2626 0%, #EF4444 100%);">
                <div class="login-form-container">
                    <div class="text-center mb-4">
                        <i class="fas fa-user-shield fa-3x text-danger mb-3"></i>
                        <h3>Login sebagai Superadmin</h3>
                        <p class="text-muted">Masukkan kredensial superadmin Anda</p>
                    </div>
                    ...
                    <?php else: ?>
                        ...
                        <form method="POST" class="login-form">
                            ...
                            <button type="submit" class="btn btn-danger w-100 btn-lg">
                                <i class="fas fa-sign-in-alt me-2"></i>Login Superadmin
                            </button>
                        </form>
                    <?php endif; ?>
                    ...
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <div class="alert alert-info mt-3" role="alert">
                            ...
                            <br>
                            <a href="login_superadmin.php?logout=1<?php echo get_tab_id() > 0 ? '&tab=' . get_tab_id() : ''; ?>" class="alert-link mt-2 d-inline-block">
                                <i class="fas fa-sign-out-alt me-1"></i>Logout dan Login sebagai Superadmin Lain
                            </a>
                        </div>
                    <?php endif; ?>
