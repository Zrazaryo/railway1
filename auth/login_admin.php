...
if ($force_logout && isset($_SESSION['user_id'])) {
    ...
    header('Location: login_admin.php' . $tab_param);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $db_configured) {
    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            // Find user with admin role - case sensitive username match
            $sql = "SELECT id, username, password, full_name, email, role, status, profile_picture 
                    FROM users 
                    WHERE BINARY username = ? AND role = 'admin' AND status = 'active'";
            ...
            if ($user) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    log_activity_with_detection($user['id'], 'LOGIN_ADMIN', 'Admin berhasil login');
                ...
                if ($check_user) {
                    if ($check_user['role'] !== 'admin') {
                        $error_message = 'Username ini bukan admin. Silakan login sebagai staff.';
...
<head>
    ...
    <title>Login Admin - Sistem Arsip Dokumen</title>
        ...
        <div class="row min-vh-100">
            ...
            <div class="col-lg-6 d-none d-lg-flex align-items-center justify-content-center" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                <div class="text-center text-white">
                    <i class="fas fa-user-tie fa-5x mb-4"></i>
                ...
                <div class="login-form-container">
                    ...
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <div class="alert alert-info mt-3" role="alert">
                            ...
                            <br>
                            <a href="login_admin.php?logout=1<?php echo get_tab_id() > 0 ? '&tab=' . get_tab_id() : ''; ?>" class="alert-link mt-2 d-inline-block">
                                <i class="fas fa-sign-out-alt me-1"></i>Logout dan Login sebagai Admin Lain
                            </a>
                        </div>
                    <?php endif; ?>
