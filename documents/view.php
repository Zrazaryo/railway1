...
// Set JSON header dan no-cache agar daftar file (foto) selalu terbaru setelah upload/edit
...
try {
    ...
    // Hitung kode dokumen (month_number.document_order_number) jika tersedia
    $kode_dokumen = '-';
    if (!empty($document['month_number']) && $document['document_order_number'] !== null) {
        $kode_dokumen = $document['month_number'] . '.' . $document['document_order_number'];
    }

    // Format waktu dibuat dokumen
    $created_at_display = '-';
    if (!empty($document['created_at'])) {
        $created_at_obj = DateTime::createFromFormat('Y-m-d H:i:s', $document['created_at']);
        if ($created_at_obj) {
            $created_at_display = $created_at_obj->format('d/m/Y H:i');
        } else {
            $created_at_display = $document['created_at'];
        }
    }

    // Generate HTML dengan layout mirip edit.php
    $html = '
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" value="' . e($kode_dokumen) . '" readonly>
                                </div>
                            </div>
                        </div>
    ...
    $html .= '
                        ...
                        <div class="mt-2">
                            <small class="text-muted">
                                Dibuat pada: ' . e($created_at_display) . 
                                (!empty($document['created_by_name']) ? ' oleh ' . e($document['created_by_name']) : '') . '
                            </small>
                        </div>
                    </div>
                </div>
    ...
    foreach ($document_types as $type_key => $type_label) {
        ...
        if (isset($files_by_type[$type_key]) && count($files_by_type[$type_key]) > 0) {
            ...
            $html .= '<option value="exists">Ada</option>';
            $html .= '</select>';
            
            // Add file previews (hanya untuk file yang benar-benar diupload, bukan status placeholder)
            $html .= '<div class="mt-2 w-100">';
            foreach ($files_by_type[$type_key] as $file) {
                // Skip jika ini adalah status placeholder (file_path = 'STATUS_ONLY')
                if ($file['file_path'] === 'STATUS_ONLY') {
                    continue; // Status "Ada" tanpa file, tidak perlu tampilkan preview
                }
            ...
            // Tidak ada record di database = status "Tidak Ada"
            $html .= '<select class="form-select" disabled>';
            $html .= '<option value="none">Tidak Ada</option>';
            $html .= '</select>';
        }
