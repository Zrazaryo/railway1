...
if (empty($selected_locker_code)) {
    ...
    header('Location: pemusnahan_lockers.php?error=' . urlencode('Silakan pilih rak terlebih dahulu sebelum menambah dokumen pemusnahan.'));
    exit();
}

// Ambil data lemari/rak dan cek kapasitas
try {
    // Cari berdasarkan nama rak, karena nama rak yang dikirim dari halaman pilih lemari pemusnahan
    $selected_locker = $db->fetch("SELECT * FROM lockers WHERE name = ?", [$selected_locker_code]);
    if (!$selected_locker) {
        header('Location: pemusnahan_lockers.php?error=' . urlencode('Kode lemari tidak valid.'));
        exit();
    }
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name)) {
        ...
        try {
                // Cek kapasitas lemari untuk dokumen pemusnahan (maksimal 800 dokumen)
                $countRow = $db->fetch(
                    "SELECT COUNT(*) AS used FROM documents WHERE month_number = ? AND status = 'deleted'",
                    [$month_number]
                );
                ...
                // Validasi sisi server untuk nomor urut dokumen manual (cek di dokumen pemusnahan)
                $checkDuplicateSql = "SELECT COUNT(*) FROM documents WHERE month_number = ? AND document_order_number = ? AND status = 'deleted'";
                $duplicateCount = $db->fetch($checkDuplicateSql, [$month_number, $document_order_number])['COUNT(*)'];
                if ($duplicateCount > 0) {
                    throw new Exception('Nomor urut dokumen ' . $document_order_number . ' sudah digunakan di rak pemusnahan ' . $month_number . '. Silakan gunakan nomor lain.');
                }
                
                // Generate nomor dokumen
                $document_number = generate_document_number();
                
            // Simpan data utama ke database dengan status 'deleted' untuk pemusnahan
            $sql = "INSERT INTO documents (document_number, title, full_name, nik, passport_number, birth_date, month_number, marriage_certificate, birth_certificate, divorce_certificate, custody_certificate, citizen_category, document_origin, document_order_number, document_year, file_path, file_name, file_size, file_type, created_by, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'deleted')";
            ...
            foreach ($document_types as $field_name => $document_name) {
                ...
                // Log aktivitas
            log_activity($_SESSION['user_id'], 'ADD_DOCUMENT_PEMUSNAHAN', "Menambah dokumen pemusnahan: $full_name ($uploaded_files file)", $document_id);
...
<head>
    ...
    <title>Tambah Dokumen Pemusnahan - Sistem Arsip Dokumen</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Tambah Dokumen Pemusnahan</h1>
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <a href="pemusnahan.php" class="btn btn-outline-secondary">
                        ...
                        <div class="card shadow">
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-text">
                                            Nama / kode rak dipilih dari halaman sebelumnya. Kapasitas tiap rak: 800 dokumen pemusnahan.
                                        </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <?php
                                            // Ekstrak kode lemari dari code field - sama seperti di select.php
                                            $locker_code_display = '';
                                            if (isset($selected_locker['code'])) {
                                                $code = $selected_locker['code'];
                                                // Jika code dimulai dengan huruf diikuti angka lalu angka lagi (format baru: A101, B102)
                                                // Contoh: A101 -> A1, B102 -> B1, Z121 -> Z1
                                                if (preg_match('/^([A-Z])(\d)(\d{2})$/', $code, $matches)) {
                                                    $locker_code_display = $matches[1] . $matches[2]; // A1, B1, Z1
                                                } elseif (preg_match('/^([A-Z])(\d+)$/', $code, $matches)) {
                                                    // Format: A1, A10, B5, dst - tampilkan langsung
                                                    $locker_code_display = $code;
                                                } elseif (preg_match('/^([A-Z])(\d{2})$/', $code, $matches)) {
                                                    // Format lama: A01, A02 -> ambil huruf pertama saja
                                                    $locker_code_display = $matches[1];
                                                } else {
                                                    // Fallback: tampilkan code langsung
                                                    $locker_code_display = $code;
                                                }
                                            }
                                            ?>
                    ...
                    <div class="row mt-4">
                        <div class="col-12">
                            <div class="d-flex gap-2 justify-content-end">
                                <a href="pemusnahan.php" class="btn btn-success">
                                    <i class="fas fa-arrow-left"></i> Kembali
                                </a>
